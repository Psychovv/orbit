  ...
  return (
    <div className="card-cosmic p-4 sm:p-5">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h4 className="font-title font-semibold text-sm text-text">Ritmo Semanal</h4>
          <p className="text-xs text-text-dim">Tarefas concluídas nos últimos 7 dias</p>
        </div>
        <div className="font-mono text-xs text-accent-plasma font-medium px-2 py-0.5 rounded-full bg-accent/10 border border-accent/20">
          {days.reduce((acc, d) => acc + d.count, 0)} total
        </div>
      </div>

      <div className="flex items-end justify-between gap-2 pt-4 h-28 border-b border-line/40 px-1">
        {days.map((day) => {
          ...
          return (
            <div key={day.dateStr} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end group">
              {/* Tooltip on hover */}
              <span className="font-mono text-[10px] text-text-dim group-hover:text-text group-hover:scale-110 transition-all opacity-0 group-hover:opacity-100 mb-0.5">
                {day.count}
              </span>
              ...
              {/* Label */}
              <span
                className={`font-mono text-[11px] pt-1 transition-colors ${
