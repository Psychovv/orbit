  ...
  return (
    <FadeUp>
      <div className="space-y-6 pb-24 max-w-[1600px] mx-auto">
        {/* Filters & Actions Bar */}
        <div className="card-cosmic p-4 sm:p-5 flex flex-wrap items-center justify-between gap-4">
          {/* Area Filter Chips */}
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-mono text-sm text-text-dim flex items-center gap-1.5 mr-1">
              <Filter size={16} />
              Filtrar:
            </span>
            ...
            <button
              ...
              className={cn(
                "px-3.5 py-1.5 rounded-full text-sm font-title font-medium transition-all",
            ...
            {state.areas.map((area) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-sm font-title font-medium transition-all",
                    isSelected
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 px-3.5 py-2 rounded-xl border text-sm font-mono transition-colors",
                hideCompleted
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4 items-start">
          {orderedWeekdays.map((weekday) => {
            ...
            return (
              <StaggerItem key={weekday}>
                <div
                  onDragOver={(e) => handleDragOver(e, weekday)}
                  onDragLeave={(e) => handleDragLeave(e, weekday)}
                  onDrop={(e) => handleDrop(e, weekday)}
                  className={cn(
                    "rounded-2xl border transition-all duration-200 flex flex-col min-h-[520px]",
                    isToday
                      ? "bg-bg-elev border-accent/60 shadow-[0_0_30px_rgba(168,85,247,0.25)]"
                      : isDraggedOver
                ...
                >
                  ...
                  <div
                    className={cn(
                      "p-4 border-b flex items-center justify-between rounded-t-2xl",
                      isToday
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span
                          className={cn(
                            "font-title font-bold text-base",
                        ...
                        {isToday && (
                          <span className="font-mono text-xs uppercase px-2 py-0.5 rounded-full bg-accent text-white font-semibold shadow-xs">
                            Hoje
                          </span>
                        )}
                      </div>
                      <span className="text-xs font-mono text-text-dim block capitalize mt-0.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-text-dim hover:text-text hover:bg-bg-elev transition-colors"
                  ...
                  <div className="p-3 space-y-4 flex-1 overflow-y-auto">
                    {blocks.length === 0 ? (
                      <div className="py-16 text-center text-sm text-text-faint font-mono">
                      ...
                      blocks.map((block) => {
                        ...
                        return (
                          <div
                            key={block.id}
                            className="rounded-xl border border-line bg-bg/60 p-3.5 space-y-3 hover:border-line-strong transition-all"
                            style={{
                              borderLeftWidth: '3px',
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between gap-2">
                              <div className="min-w-0">
                                <span
                                  className="font-mono text-xs uppercase font-semibold block truncate"
                                  style={{ color: area?.color }}
                                >
                                  {area?.label}
                                </span>
                                <h5 className="font-title font-bold text-sm text-text truncate">
                                  {block.title}
                                </h5>
                              </div>
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-md text-text-dim hover:text-accent-plasma hover:bg-bg-elev transition-colors"
